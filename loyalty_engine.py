...
def normalize_custom_command_rule(rule: Any) -> Optional[dict[str, Any]]:
    ...
    return {
        ...
        "description": str(rule.get("description", "")).strip()[:200],
...
def _public_command(
    ...
    return {
        "command": f"!{_clean_user(command)}",
        "description": str(description).strip(),
        "category": category,
        "permission": permission or "everyone",
        "cost": max(0, int(cost or 0)),
        "aliases": [f"!{_clean_user(alias)}" for alias in aliases or [] if _clean_user(alias)],
        "cooldown_seconds": max(0, int(cooldown_seconds or 0)),
        "user_cooldown_seconds": max(0, int(user_cooldown_seconds or 0)),
    }


def build_public_command_catalog(settings, active_raffle: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    """Return viewer-safe command metadata without secrets or local implementation details."""
    commands: list[dict[str, Any]] = [
        _public_command(settings.cmd_sr, "Request a song by URL or search term.", "Song Requests"),
        _public_command(settings.cmd_queue, "Show the current song request queue.", "Song Requests"),
        _public_command(settings.cmd_wrongsong, "Remove your most recent queued request.", "Song Requests"),
    ]

    moderator_commands = [
        (settings.cmd_skip, "Skip the current media."),
        ("stop", "Stop the media player without clearing the queue."),
        (settings.cmd_pause, "Pause or resume the current media."),
        (settings.cmd_play, "Resume paused media or restart the stopped queue."),
        (settings.cmd_hide, "Hide or show the song request window."),
        (settings.cmd_show, "Show the song request window."),
        (settings.cmd_full, "Toggle fullscreen media."),
        (settings.cmd_info, "Toggle the song title overlay."),
        (settings.cmd_clearqueue, "Clear the request queue."),
        (settings.cmd_sron, "Enable song requests."),
        (settings.cmd_sroff, "Disable song requests."),
    ]
    commands.extend(
        _public_command(command, description, "Moderator Controls", permission="mod")
        for command, description in moderator_commands
        if command
    )
    ...
    if settings.loyalty_enabled:
        commands.extend(
            [
                _public_command(
                    settings.cmd_balance,
                    f"Check your {settings.currency_name} balance.",
                    "Loyalty",
                ),
                _public_command(
                    settings.cmd_leaderboard,
                    f"Show the top {settings.currency_name} leaderboard.",
                    "Loyalty",
                ),
                _public_command(
                    settings.cmd_give_points,
                    f"Give {settings.currency_name} to another viewer.",
                    "Loyalty",
                ),
            ]
        )
        commands.extend(
            [
                _public_command(
                    settings.cmd_add_points,
                    f"Add {settings.currency_name} to a viewer.",
                    "Moderator Controls",
                    permission="mod",
                ),
                _public_command(
                    settings.cmd_remove_points,
                    f"Remove {settings.currency_name} from a viewer.",
                    "Moderator Controls",
                    permission="mod",
                ),
            ]
        )
        if settings.gambling_enabled:
            commands.append(
                _public_command(
                    settings.cmd_gamble,
                    f"Gamble {settings.currency_name}.",
                    "Games",
                    cooldown_seconds=settings.gamble_cooldown_seconds,
                )
            )
        if settings.duels_enabled:
            commands.extend(
                [
                    _public_command(
                        settings.cmd_duel,
                        f"Challenge another viewer to a {settings.currency_name} duel.",
                        "Games",
                        cooldown_seconds=settings.duel_cooldown_seconds,
                    ),
                    _public_command(settings.cmd_duel_accept, "Accept a pending duel.", "Games"),
                    _public_command(settings.cmd_duel_decline, "Decline a pending duel.", "Games"),
                ]
            )

    if settings.raffle_enabled:
        raffle_title = ""
        if isinstance(active_raffle, dict):
            raffle_title = str(active_raffle.get("title", "")).strip()
            raffle_command = str(active_raffle.get("entry_command", "")).strip()
        else:
            raffle_command = ""
        command = raffle_command or settings.cmd_raffle_enter
        description = (
            f"Enter the active raffle: {raffle_title}."
            if raffle_title
            else "Enter the active raffle when one is running."
        )
        commands.append(_public_command(command, description, "Raffles"))

    if settings.automation_enabled:
        for raw_rule in settings.custom_commands:
            rule = normalize_custom_command_rule(raw_rule)
            if rule is None or not rule.get("enabled", True):
                continue
            description = rule.get("description") or "Custom stream command."
            commands.append(
                _public_command(
                    rule["name"],
                    description,
                    ...
                    user_cooldown_seconds=rule["user_cooldown_seconds"],
                )
            )

    commands.sort(key=lambda item: (item["category"].casefold(), item["command"].casefold()))
    categories = sorted({item["category"] for item in commands}, key=str.casefold)
    return {
        "ok": True,
